...
def train_complex(config: DictConfig) -> None:
    ...
    total_samples = volumes.return_real_samples()
    total_samples= [fft2(sample.t1.data) for sample in total_samples]
    print(len(total_samples))
    torch.manual_seed(config.global_seed)
    shuffle(total_samples)
    load_model = config.load_model

    class_weights = torch.FloatTensor([3.54,1,1]).to(config.device)#for dataset being unbalanced for classes [LGG, HGG, Healthy]
    ...
    predlist = torch.zeros(0, dtype=torch.long).to(config.device)
    lbllist = torch.zeros(0, dtype=torch.long).to(config.device)
    ...
    for epoch in range(config.training.num_epoch):
        ...
        for i, traindata in enumerate(trainloader):
            images = F.interpolate(traindata['t1'][torchio.DATA], scale_factor=(config.dataset.img_scale_factor,config.dataset.img_scale_factor,config.dataset.img_scale_factor)).to(config.device)
            labels = traindata['label'].to(config.device)
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            for testdata in testloader:
                ...
                images = F.interpolate(testdata['t1'][torchio.DATA], scale_factor=(config.dataset.img_scale_factor,config.dataset.img_scale_factor,config.dataset.img_scale_factor)).to(config.device)

                labels = testdata['label'].to(config.device)
                outputs = model(images)
